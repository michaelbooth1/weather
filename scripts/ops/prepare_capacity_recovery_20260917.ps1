# Metadata-only preparation of the owner's exact September 17 capacity successor.
param(
...
if(-not(Test-WeatherCapacityPreparationWindow -NightDate '2026-09-17')){throw 'Preparation is too late for reviewed preflight'}
...
$c=$oldConfig.Value
$audit=Read-WeatherPlainMetadata (Join-Path $handoffs 'overnight-audit-20260916-0905-proof.json') 'c65f6f1cff2390e5a4c4f5380ffb1df727fe792ba483306abc784603a3c770e3' 16384
if($audit.Value.archive_payload_attempts -ne 0 -or $audit.Value.archive_originals_deleted -ne 0 -or $audit.Value.all_recorded_teardowns_proved -ne $true){throw 'Predecessor disposition differs'}
# The whole interrupted July 22 group is held, including all 24 completed paths.
# Prior savings are not carried into this disjoint successor.
...
$total=0
foreach($group in @($selection.Value.groups | Select-Object -Skip 8)){
...
if($total -ne 8545 -or $groups.Count -ne 54){throw 'Untouched compression selection accounting differs'}
$package=Join-Path $handoffs ('capacity-150gb-20260917-'+$AttemptLabel)
if(Test-Path -LiteralPath $package){throw 'Spent preparation namespace'}
$baselinePrefix=Join-Path $handoffs ('capacity-20260917-cap150-'+$AttemptLabel)
...
$ledger=Write-WeatherPlainNew ($baselinePrefix+'-baseline-files.json') @{
 ...
 scope='Zero credit for this new attempt. Only the bound untouched groups 8 onward are eligible; previous eight groups are excluded.'
...
$plan=@{
 schema_version='storage_recovery_night_plan_v1';plan_id='capacity-20260917-cap150';night_date='2026-09-17'
 approved_by='Michael; September 15 exact capacity-recovery approval';approved_at_utc='2026-09-15T23:17:20.425091Z'
 expires_at_utc='2026-09-17T13:00:00Z';production_repo_root=$ProductionRepoRoot;execution_host_id=$hostId
 ...
 target_new_reclaimed_bytes=1;target_free_disk_bytes=150000000000;max_new_files=8545;max_wrapper_attempts=448
...
$exception=Write-WeatherPlainNew (Join-Path $package 'disk-exception.json') @{
 ...
 plan_sha256=$c.plan.sha256;selection_sha256=$c.selection.sha256;expires_at_utc='2026-09-17T13:00:00Z'
 hard_reserve_bytes=26843545600;output_cap_bytes=2147483648;capture_bytes_per_second=1048576
} 16384
$c.campaign_id='plain-20260917-cap150';$c.source_tip=$ExpectedSourceTip;$c.created_at_utc=[DateTime]::UtcNow.ToString('o')
$c.approved_by='Michael; exact post-denial approval on September 15';$c.approved_at_utc='2026-09-15T23:17:20.425091Z'
$c.start_utc='2026-09-17T04:30:00Z';$c.end_utc='2026-09-17T08:42:00Z';$c.expires_at_utc=$c.end_utc
...
$c|Add-Member -NotePropertyName capacity -NotePropertyValue @{
 ...
 result_root=(Join-Path $ProductionRepoRoot 'scratch/storage_recovery_nights/capacity-20260917-cap150')
} -Force
foreach($row in $c.queue){$row.archive_id='p17a'+$row.chunk_id.Substring(6);$row.start_at='stage'}
