# Attended, exact-request cold cache compression. No traversal or deletion.
...
$lease = Enter-WeatherHeavyWorkloadLease -RepoRoot $ProductionRepoRoot `
    -Workload 'replay_cache_compression' -ExpectedExecutionHostId $hostIdentity
...
$oldSource = $env:WEATHER_CACHE_COMPRESSION_SOURCE_ROOT
$oldOwner = $env:WEATHER_CACHE_COMPRESSION_OWNER_PID
$oldDeadline = $env:WEATHER_CACHE_COMPRESSION_DEADLINE_UTC
$receipt = [ordered]@{
...
try {
    ...
    $env:WEATHER_CACHE_COMPRESSION_SOURCE_ROOT = $sourceRoot
    $env:WEATHER_CACHE_COMPRESSION_OWNER_PID = [string]$PID
    $env:WEATHER_CACHE_COMPRESSION_DEADLINE_UTC = $deadline.ToString('o')
    $arguments = @('-m', 'weather.operations.replay_cache_compression',
    ...
    try {
        ...
        $env:WEATHER_CACHE_COMPRESSION_SOURCE_ROOT = $oldSource
        $env:WEATHER_CACHE_COMPRESSION_OWNER_PID = $oldOwner
        $env:WEATHER_CACHE_COMPRESSION_DEADLINE_UTC = $oldDeadline
