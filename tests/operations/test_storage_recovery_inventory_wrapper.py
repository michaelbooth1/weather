...
def wrapper_fixture(tmp_path, request):
    ...
    admission = admission.replace("Global\\WeatherProjectHeavyWorkloadV1", "Local\\InventoryFixture-" + uuid.uuid4().hex)
    ...
    (package / "storage_recovery_inventory_cli.py").write_text(CHILD)
...
def launch(wrapper_fixture, mode, *, apply=False, plan_receipt=None, exception=""):
    ...
    output = production / "scratch/storage_recovery_inventory" / ("apply" if plan_receipt else "attempt")
