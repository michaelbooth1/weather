...
def test_compact_scratch_supports_long_root_and_preserves_other_attempts(
    ...
    other_attempt = repo_root / ".e" / ("a" * 40)
    ...
    # Reconciled layout: the run root is the workspace, named
    # <8 hex of the manifest hash><32 hex uuid> under <repo>/.e.
    run_root = workspace
    assert run_root.parent == repo_root / ".e"
    assert len(run_root.name) == 40
    assert run_root.name.startswith(manifest["manifest_sha256"][:8])
    ...
    assert list((repo_root / ".e").iterdir()) == [other_attempt]
