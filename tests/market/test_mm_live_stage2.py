...
def platform_gate():
    return {
        ...
        "checks": {
            "complete_v0_4": True,
            "stage1_derived_evidence_matches_platform_fields": True,
        },
        "missing": [],
    }
...
def paper_counterfactual(*, generated=NOW):
    ...
    payload = {
        "schema_version": "mm_live_stage2_paper_counterfactual_v0.2",
...
def market_harvest_quote_decision(**updates):
    row = quote_decision()
    row.update({
...
def market_harvest_preflight():
    preflight = market_preflight()
    preflight["permission_profile"] = "market_harvest"
    preflight["gates"] = [
        gate
        for gate in preflight["gates"]
        if gate["name"]
        not in {"snapshot_model_rows", "model_freshness", "promotion_state"}
    ]
    preflight["gates"].append({
        "name": "market_harvest_paper_only",
        "ok": True,
    })
    preflight["live_gate"] = {
        "required": False,
...
def market_harvest_counterfactual(**updates):
    row = market_harvest_quote_decision(**updates)
    payload = {
        "schema_version": "mm_live_stage2_paper_counterfactual_v0.2",
...
def test_stage2_accepts_only_profile_bound_market_harvest_paper_proof():
    ...
    assert envelope["schema_version"] == "mm_live_stage2_session_envelope_v0.2"
    assert envelope["source_permission_profile"] == "market_harvest"
    assert envelope["source_run_mode"] == "paper-live-forward"
    assert envelope["source_live_trade_permission"] is False
    assert envelope["source_shadow_mode"] is True
    assert envelope["ordinary_model_lane_unchanged"] is True
    assert envelope["model_promotion_bypass_scope"] == (
        "stage2_single_submit_from_market_harvest_paper_proof"
    )


def test_stage2_accepts_the_actual_market_harvest_tape_shape():
    row = market_harvest_quote_decision(policy_hash="a" * 16)
    row.pop("permission_profile")
    row.pop("daily_loss_pusd")
    artifact = json.dumps({
        "schema_version": "mm_live_stage2_paper_counterfactual_v0.2",
        "artifact_recorded_at_utc": NOW.isoformat(),
        "quote_row": row,
    ...
    assert envelope["source_permission_profile"] == "market_harvest"
    assert envelope["daily_loss_before_pusd"] == "0"
    assert envelope["daily_loss_before_basis"] == (
...
@pytest.mark.parametrize(
    "quote_updates,error",
    [
        ({"live_trade_permission": True}, "live_permission"),
        ({"run_mode": "live-pilot"}, "harvest_mode"),
        ({"known_edge_permission": "full"}, "profile marker"),
        ({"model_variant_probability_source": "served_model"}, "harvest_marker"),
        ({"side": "BUY"}, "two_sided"),
        ({"expected_rebate_value": 0.01}, "zero_reward"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda gate: gate["sdk_contract"].update({
        ...
        (
            lambda gate: gate["dead_man_heartbeat"].update({
                "endpoint": "/v1/heartbeats",
                "request_body_absent_verified": False,
            }),
            "heartbeat_contract",
        ),
    ],
)
def test_stage2_rejects_retired_sdk_and_heartbeat_contracts(
    tmp_path,
    mutate,
    missing,
):
    ...
    with pytest.raises(RuntimeError, match=missing):
        execute_stage2_maker_session(
            ...
            confirmation=CONFIRMATION,
            session_budget_pusd=25,
            journal_path=tmp_path / f"retired-{missing}.jsonl",
            wall_clock=lambda: NOW,
        )

    assert adapter.submit_calls == 0
    assert not (tmp_path / f"retired-{missing}.jsonl").exists()
...
def test_official_adapter_stage2_capability_is_buy_only_and_single_submit():
    ...
    adapter = OfficialPolymarketGlobalAdapter(
        ...
        market_rule_reader=market_rules,
        sdk_version="0.6.0",
    ...
    retired_sdk_gate = platform_gate()
    retired_sdk_gate["sdk_contract"].update({
        "distribution": "py-clob-client-v2",
        "version": "1.1.0",
    })
    with pytest.raises(RuntimeError, match="sdk"):
        adapter.authorize_stage2_maker_session(retired_sdk_gate, envelope)
    retired_heartbeat_gate = platform_gate()
    retired_heartbeat_gate["dead_man_heartbeat"].update({
        "endpoint": "/v1/heartbeats",
        "request_body_absent_verified": False,
    })
    with pytest.raises(RuntimeError, match="heartbeat"):
        adapter.authorize_stage2_maker_session(retired_heartbeat_gate, envelope)
