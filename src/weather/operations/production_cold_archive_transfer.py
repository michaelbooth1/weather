...
def validate_request(payload, *, production_root, now, source_git_sha):
    ...
    if (payload["schema_version"] != schema_version("production_cold_archive_transfer_request")
            or payload["operation"] != "upload_and_independent_download"):
    ...
    for field in (*PATH_FIELDS, "plan_path"):
...
def _run_pinned(args, root, output, request_path):
    ...
    staging.validate_chunk(plan, request["chunk_id"], root, now)
    ...
    if reread != result or result.get("status") != "PASS" or result.get("independent_download") is not True:
    ...
    final = {"schema_version": schema_version("production_cold_archive_transfer_execution_receipt"),
             ...
             "transport_receipt_sha256": hashlib.sha256(receipt_raw).hexdigest(),
             "transport_receipt_path": str(receipt_path), "source_retained": True,
             "deleted_files": 0, "reclaimed_bytes": 0, "cleanup_eligible": False,
             "upload_performed": True, "independent_download": True, "final_admission": last_admission}
...
def main(argv=None):
    ...
    parser.add_argument("operation", choices=["transfer"])
