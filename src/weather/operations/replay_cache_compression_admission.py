...
# Owner timing correction on September 13: execute the approved transfer now.
ARCHIVE_IMMEDIATE_EXCEPTION = "OWNER_APPROVED_ARCHIVE_RECOVERY_20260913_EVENING"
ARCHIVE_IMMEDIATE_START = datetime(2026, 9, 13, 22, 46, tzinfo=timezone.utc)
ARCHIVE_IMMEDIATE_END = datetime(2026, 9, 14, 4, 30, tzinfo=timezone.utc)
...
STORAGE_DAYTIME_EXCEPTIONS = {
    ARCHIVE_IMMEDIATE_EXCEPTION: ("2026-09-13", ARCHIVE_IMMEDIATE_EXCEPTION.lower()),
...
def storage_daytime_authorized(now, exception):
    if exception == ARCHIVE_IMMEDIATE_EXCEPTION:
        return ARCHIVE_IMMEDIATE_START <= now < ARCHIVE_IMMEDIATE_END
