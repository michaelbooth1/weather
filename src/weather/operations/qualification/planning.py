...
def draft(git, *, output, production, candidate, reviewed, qualification, environment, metadata):
    ...
    graph = Graph(Path(qualification["root"]))
    local = Graph(output)
    closure = local.get(closure_ref)
    hooks = output / "empty-hooks"
    hooks.mkdir()
    policy = git_policy.capture(git, production, candidate, baseline=baseline, commit=source_commit,
        hooks=hooks, graph=graph, environment_ref=environment["environment"], bindings=environment["bindings"])
    policy_ref = publish(output, "git-policy.json", policy)
    value = {"schema": "weather_integration_attempt_manifest_v2", "qualification_mode": "split_v2", **metadata,
        ...
        "qualification": qualification, "control": {"root": str(control), "closure": closure_ref, "git_policy": policy_ref}}
...
def run(request_path, expected_sha256):
    ...
    require(request["operation"] in {"configuration", "draft"}, "unknown fixed planning operation")
