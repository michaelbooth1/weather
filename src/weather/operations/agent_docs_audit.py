...
def _markdown_outside_fenced_code(text: str) -> str:
    """Blank fenced code while preserving prose and line boundaries.

    PowerShell casts such as ``[string](...)`` look like Markdown links but are
    executable examples.  Both CommonMark fence characters and longer closing
    fences are supported without trying to parse the rest of Markdown.
    """
    ...
    for line in text.splitlines(keepends=True):
        ...
        if marker == fence[0] and run_length >= fence[1] and not candidate[run_length:].strip():
            fence = None
        visible.append("\n" if line.endswith(("\n", "\r")) else "")
    return "".join(visible)
...
def broken_local_links(repo_root: Path, paths: list[Path] | None = None) -> list[str]:
    ...
    for path in paths or _markdown_files(repo_root):
        ...
        text = _markdown_outside_fenced_code(path.read_text(encoding="utf-8-sig"))
