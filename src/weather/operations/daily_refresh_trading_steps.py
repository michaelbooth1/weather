...
def run_maker_paper_score_step(args):
    ...
    try:
        companion_folders = market_harvest_companion.discover_run_folders(runs_root)
        if companion_folders:
            result["market_harvest_companion"] = market_harvest_companion.score_runs(
                ...
                snapshots_root=getattr(
                    args,
                    "snapshots_root",
                    mm_paper.DEFAULT_SNAPSHOTS_ROOT,
                ),
                backtest_root=backtest_root,
                selected_run_folders=companion_folders,
                latest_n=latest_runs,
                now=getattr(args, "as_of", None) or utc_iso(),
                casebook_path=backtest_path(args, "disagreement_casebook.json"),
                ledger_root=getattr(args, "ledger_root", None),
                ...
                exchange_economics_target_date=(
                    settled_analysis_target_date(args).isoformat()
                ),
                exchange_economics_platform=getattr(
                    args,
                    "exchange_economics_platform",
