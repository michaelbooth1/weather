...
ENVELOPE_SCHEMA_VERSION = "mm_live_stage2_session_envelope_v0.1"
...
PAPER_COUNTERFACTUAL_SCHEMA_VERSION = (
    "mm_live_stage2_paper_counterfactual_v0.1"
)
...
REQUIRED_STAGE2_PREFLIGHT_GATES = frozenset({
    "active_event",
    "snapshot_model_rows",
    "model_freshness",
    "source_status_rows",
...
def _gate_binding(adapter, platform_gate, session_budget):
    ...
    binding = {
        ...
        "heartbeat_endpoint": (
            (gate.get("dead_man_heartbeat") or {}).get("endpoint")
            == "/v1/heartbeats"
        ),
        "heartbeat_chain": all((
            (gate.get("dead_man_heartbeat") or {}).get("acknowledgment_verified") is True,
            (gate.get("dead_man_heartbeat") or {}).get("rotating_id_chain_verified") is True,
            (gate.get("dead_man_heartbeat") or {}).get("automatic_cancel_verified") is True,
        )),
...
def _paper_counterfactual_binding(live_row, evidence, *, current):
    raw_artifact, payload = _json_object_bytes(
        evidence,
        "Stage 2 paper counterfactual artifact",
    )
    paper_row = dict(payload.get("quote_row") or {})
    live_generated = _parse_utc(
        live_row.get("generated_at_utc"),
        "live quote generated_at_utc",
    )
    ...
    exact_text = all(
        str(paper_row.get(field) or "") == str(live_row.get(field) or "")
    ...
    try:
        ...
        exact_numbers = False
    fair = _decimal(live_row.get("fair_probability"), "live fair probability")
    required_identity = all((
        bool(str(live_row.get("event_slug") or "")),
        bool(str(live_row.get("snapshot_id") or "")),
        bool(str(live_row.get("served_model_version") or "")),
        _is_sha256(live_row.get("policy_hash")),
        fair < 1,
    ))
    checks = {
        ...
        "live_generated_before_freeze": live_generated <= current,
...
def build_stage2_session_envelope(
    ...
    *,
    ...
    """Validate one current quote decision and freeze the non-raisable envelope."""
    ...
    paper_binding = _paper_counterfactual_binding(row, paper_counterfactual, current=current)
    capture_binding = _public_capture_binding(row, public_capture_evidence, current=current)
    ...
    event_before = _decimal(row.get("event_notional"), "event notional", allow_zero=True)
    band_before = _decimal(row.get("band_notional"), "band notional", allow_zero=True)
    daily_loss_before = _decimal(
        row.get("daily_loss_pusd"),
        "daily loss",
        allow_zero=True,
    )
    ...
    checks = {
        ...
        "preflight_gates": isinstance(preflight_gates, list)
        and bool(preflight_gates)
        and all(gate_row.get("ok") is True for gate_row in preflight_gates),
        "required_preflight_gates": REQUIRED_STAGE2_PREFLIGHT_GATES.issubset(
            preflight_gate_names
        ),
        "live_gate": all((
        ...
        "market": str(preflight.get("market_id") or "")
        == str(row.get("market_id") or "")
        and bool(str(row.get("market_id") or "")),
        "target": str(preflight.get("target_date") or "")
        ...
        "run_budget": run_budget == budget and budget <= MAX_OPERATOR_BUDGET_PUSD,
    ...
    missing = [name for name, valid in checks.items() if not valid]
    if missing:
        raise RuntimeError("Stage 2 quote/preflight binding failed: " + ", ".join(missing))
    envelope = {
