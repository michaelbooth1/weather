...
def collect_project_overview(repo_root=REPO_ROOT):
    ...
    # STATE_OF_PLAY.md was restructured on 2026-09-19: "**Objectives, in order:**" and
    # "**Last updated: ...**". Accept both the old and the current spellings.
    objective = re.search(
        r"\*\*Objectives?(?:, in order)?:\*\*\s*(.*?)(?:\n\n|\Z)", state, re.DOTALL
    )
    updated = re.search(r"\*\*Last (?:rewritten|updated):\s*(.*?)\*\*", state)
