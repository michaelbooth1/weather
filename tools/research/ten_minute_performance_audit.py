"""Settlement-scored 10-minute model performance audit.

This is a research report generator. It reuses the production backtest scoring
path, then changes only the time aggregation from hourly checkpoints to
...
SCHEMA_VERSION = "ten_minute_model_performance_audit_v0.1"
DEFAULT_JSON_OUT = data_path() / "backtest" / "ten_minute_model_performance_audit.json"
DEFAULT_REPORT_OUT = data_path() / "backtest" / "ten_minute_model_performance_audit.md"
...
def build_candidate_item147(path: Path) -> dict[str, Any]:
...
def build_payload(args: argparse.Namespace) -> dict[str, Any]:
    labels, skipped = discover_labeled_folders(
        ...
        quality_grades=tuple(args.quality_grades.split(",")) if args.quality_grades else DEFAULT_QUALITY_GRADES,
    ...
    candidate = build_candidate_item147(Path(args.item147_rows)) if args.item147_rows else {}
    if candidate:
        candidate["weak_slot_overlap"] = candidate_weak_slot_overlap(candidate, weak_slots)
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    if candidate.get("available"):
        ...
        lines.append(
            ...
            f"and market {fmt_num(early.get('market_brier'))}. On overlapping weak slots, its weighted "
            f"Brier delta versus current is {fmt_signed(overlap.get('weighted_delta_vs_current'))}."
        )
    else:
    ...
    if candidate.get("available"):
        ...
        lines += markdown_table(
            ...
            [
                ...
                ["Weighted delta vs current", fmt_signed(overlap.get("weighted_delta_vs_current"))],
                ["Weighted delta vs market", fmt_signed(overlap.get("weighted_delta_vs_market"))],
            ],
        )
    lines += [
        "",
...
def write_outputs(payload: dict[str, Any], args: argparse.Namespace) -> tuple[Path, Path, Path, Path | None]:
    json_out = Path(args.json_out)
    report_out = Path(args.report_out)
    slot_csv_out = Path(args.slot_csv_out)
    ...
    candidate = payload.get("candidate_item147") or {}
    if candidate.get("available"):
        candidate_csv = write_csv(candidate.get("by_slot") or [], Path(args.candidate_csv_out), CANDIDATE_CSV_COLUMNS)
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_out, report_out, slot_csv_out, candidate_csv = write_outputs(payload, args)
    corpus = payload.get("corpus") or {}
    overall = payload.get("overall") or {}
    weak = payload.get("weak_slots") or {}
