...
| `weather.operations.daily_refresh_stage_manifests` | Operations | Atomic stage-manifest publication, trigger disposition, exact Stage-A binding, and overnight Stage-B start gates. | Owner module; must not import the `daily_refresh` facade or step adapters. |
