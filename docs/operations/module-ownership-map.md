...
| `weather.reporting.data_layer_audit` | Reporting | Compatibility facade for historical/source audit, gates, recommendations, payload assembly, and CLI. Snapshot collection and low-fill classification live in `weather.reporting.data_layer_audit_collectors`; Markdown rendering lives in `weather.reporting.data_layer_audit_report`; remediation payload assembly lives in `weather.reporting.data_layer_audit_remediation`. | Split complete for project structure Phase 3. |
| `weather.reporting.data_layer_audit_collectors` | Reporting | Snapshot folder scanning, sidecar eligibility, artifact presence, source-status collection, and low-fill field classification. | Owner module; must not import the `data_layer_audit` facade. |
