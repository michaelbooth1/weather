...
| :--- | :--- | :--- | :--- |
| `canonical_evidence` | Append-only or source-of-truth evidence that cannot be safely rebuilt later. | Snapshot JSONL, replay inputs, shared raw forecast payloads, settlement ledgers, market-making lifecycle and risk ledgers, dedicated maker execution tapes and session receipts, taker run ledgers, raw CLOB books, websocket messages, price-history source evidence, CLOB token maps. | Permanent archive. Local deletion requires a reviewed cleanup manifest with exact paths, reason, operator, and checksums. |
...
The dedicated maker execution family consists of `mm_execution_tape.jsonl`,
`mm_execution_tape.csv`, and `mm_execution_tape_sessions.jsonl` in an event-day
snapshot folder. All three are permanent `canonical_evidence`, including the
normalized CSV: its exact prefix bytes are bound by session receipts, so it is
not an `analysis_projection` and is never eligible for projection cleanup.
