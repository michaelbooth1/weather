...
challenger is deliberately simple: a regularized, market-aware NWP residual or
ordinal model that tests genuinely new issue-qualified information against the
refitted incumbent. Complexity is not a substitute for an independent signal.
...
1. run the supported base retrain as a correctness and seasonal-baseline test;
2. build a generated model bill of materials and repair loaded-process identity;
3. qualify the full candidate and its inherited stages on the same untouched
   PIT surface, retiring stages that do not pay their way;
4. fit the simple new-information challenger using the staged Previous Runs
   fields;
5. freeze a winning graph and collect immutable shadow evidence;
6. consider promotion only through the existing release and evidence gates.

See [roadmap item 329](../roadmap/items/item-329-model-bom-loaded-identity-and-pit-challenger.md)
