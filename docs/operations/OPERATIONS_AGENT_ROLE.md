...
- **Roll sensitivity is the loaded-module closure, not a glob.** Run
  ...
  never merge those inside 12:00–18:00. Exact exit-0 roll-free tips may use the canonical
  09:00–18:00 control-plane lane, which is serialized behind Stage A by the shared lease. That
  lane is for Git integration and bounded health/evidence checks only; it is not a midday heavy
