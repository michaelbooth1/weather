...
> stopped being current â€” cut it**; stale content here is believed rather than ignored.

**Objectives:** 1. protect the Toronto capture streak Â· 2. **find a model that beats the market â€”
we do not** Â· 3. the market-making bot is the end goal.

## Where the model actually stands

**The cool bias is seasonal coverage, not the weather** (`-09-31a`, Â§2). The archive holds
**May 10 â€“ Jun 30 only, every year**, so in August **every date we serve is out-of-season** and the
first retrain blocks at **0 / 12,600 cells**.

**But the gap does not vanish in-season** (`-09-34a`, Â§4e): served in-season **1.4233x, excluding
1.0** â€” nearly unbiased, still losing, on a **resolution** deficit. The seasonal contrast itself is
**not powered**; claim no direction from it.

**So bias and sharpness are separate problems and we have a fix for one** â€” the critical path:
...
Â§1 says recalibration cannot supply. So the retrain is **necessary, not sufficient**: promise no gap
closure and cite no Brier or P&L gain. Intervals and power live in Â§2 / Â§4d / Â§4e â€” **cite them
from there, never from here.**

## Decided â€” do not relitigate without new evidence

- **Release #1 is DEFERRED** until a retrained candidate exists
  ([why](release-one-deferred-until-a-retrained-candidate.md)) â€” it would freeze artifacts
  measured a full degree cool on the dates we serve. The lock does **not** expire; the 7-day rule
  is rolling source recency.
- **Free-tier Open-Meteo only, no paid API. Training population 2021â€“2025.** Closed; **do not
...
## Do not redo these â€” they are answered

- **The free-source "blindness" repair is NO-GO** (`-09-26a`, Â§4b).
- **Release #1 is not sufficient for promotion**; whether it is *necessary* is unestablished (Â§9).
- **Do not tune the severe-tail band-suppression lever before the retrain** (Â§4d).

## In flight

| Ref | What | State |
| --- | --- | --- |
| `-09-29a`, `-09-31a`, `-09-32a`, `-09-34a`, `-09-36a` | chain split, seasonal Ã—2, severe tail, resolution | **ALL MERGED 2026-08-07** |
...
| `fix-wu-404` | scraper 404 misclassification | roll-sensitive â€” **does not fix the outage** |
...
â€” several unmerged branches are held deliberately).

## Open, unowned

- **THE SETTLEMENT SOURCE IS DOWN** â€” WU returns 404 for *every* date, including ones already
...
- **Resolution / sharpness** â€” still the larger half of the gap; `-09-36a` localised only ~7% of
...
- **4 tests fail on master, unowned** â€” `test_afternoon_residual_centering`, `test_long_job_guard`
  Ã—2, `test_tracked_artifact_manifests_match_current_repository_identity`. `pytest -q` is red
  before you start: **diff against these four before believing your change broke something.**

## Daily reads

`data/alerts/STALENESS_SWEEP.md` (**"should this have refreshed by now?"**, 08:10) Â·
`data/alerts/MORNING_BRIEFING.md` (host health) Â· `data/backtest/daily_refresh_report.md` (chain).
Use `scripts\ops\roll_verdict.ps1 -Branch <b>` for merge timing; **never derive it by hand.**
**Two standing alarms are expected, not incidents** â€” `WeatherTrainingWindow` exit **2** and the
chain's exit **1**; both proven benign in
[RETRACTED_AND_FALSE_LEADS.md](RETRACTED_AND_FALSE_LEADS.md) Â§3. Read it before escalating one.

## Update this file when

A decision changes, the critical path moves, or a mission returns. **Rewrite the affected
lines â€” do not append.** If you are adding rather than replacing, ask what became untrue.
