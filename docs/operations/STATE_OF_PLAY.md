...
**Last rewritten: 2026-08-20 21:05 America/Toronto.** Read this first, then
...
| Integration procedure | Item 329 redesigns the failed whole-night freeze as immutable per-attempt evidence, reviewed single-successor recovery, and hash-bound merge/downstream proof. Three adversarial audits found and drove repairs for the original PASS-path/recovery gaps, eight follow-ups, and finally a silent power-loss interruption plus three bounded Scheduler/evidence hardening defects. The isolated branch now distinguishes running, interrupted, never-run, and spent-merge states; preserves non-authorizing reconciliation; emits invariant evidence timestamps; and resolves only Scheduler's stale transient result from immutable PASS plus full validation. Sixty-seven focused contracts are present; all 13 PowerShell files parse and pass the expanded semantic sweep, the changed Python tests compile, diff check passes, and direct status/wait/task-validation/culture probes pass. Focused pytest execution is pending because this patch was written inside the protected 18:00-00:30 window. The exact full suite, merge, and explicit scheduler adoption remain open; no task has been registered or started. |
