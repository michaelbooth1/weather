...
**Last updated: 2026-09-21 America/Toronto (the reliability stack qualified on the host and merged; master published; workstation missions 79a, 80a, 80b, 81a, 82a, 83a, 83b and 83c handed back).**
...
`codex/integrate-1-research-20260921` (the research stack) is ready for host qualification; layers 2 and 3 fail
collection because the 82a trace tool imports a parser helper the repair renamed.
Open missions: `2026-09-83d` (bind that trace to parser version 1, then the first full suites on layers 2 and 3)
and `2026-09-84b` (the attended reward-test script), which has the workstation first.
...
5. Workstation: mission `2026-09-83d` (Forecast lead row). Production, in a heavy window: the exact
   census of chosen NBM period by cycle and local hour from the event-day payload manifests. Land the
   research stack as `codex/integrate-1-research-20260921` once the merge marker is reconciled
   (the roll tool decides its class; the workstation could not).
