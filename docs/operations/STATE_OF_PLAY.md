...
| Off-site qualification | Signed-in v3 passed all 59 native checks and real local staging at September 5 10:52:24 Toronto on repair `5e9b60e9d9f346346c0d8ef7de751fc43130d402` ([PR 22](https://github.com/michaelbooth1/weather/pull/22)). All 13 staging checks passed; separate workstation verification confirmed both self-hashes and the ciphertext hash. The manifest and receipt are in private Drive storage. The intact ciphertext exceeds the connector's upload cap, and browser file selection is blocked; manual file selection is required. The independent restore CLI is published in [PR 23](https://github.com/michaelbooth1/weather/pull/23); item 325 owns its qualification and immutable pins. Real download/restore, production identity and reclaim remain unproved. Preserve v1-v3 evidence and the no-cleanup/deletion flags. |
...
1. Complete manual Drive upload of the verified v3 ciphertext under item 325,
   then independently download and qualify its restore. Preserve v1-v3 evidence.
