...
| --- | --- |
| Production source | Local `master` = `e28530af6` (2026-09-21 02:31): the reliability stack (`codex/reliability-host-qual-20260919` = PR 61 + audit fixes + host fixtures) merged through the guarded tool with capture and execution-tape recovery proved. **Not yet published: `origin/master` is still `84060d0c5`** - `WeatherOneShotPush` is an interactive-logon task and did not run (nobody has logged on at the console since the 09-20 11:14 restart - likely cause, not verified); an agent fast-forward push was refused by the permission layer. **Owner action: log on and run the push task, or `git push origin master`.** Production runs the local tree, so nothing is at risk meanwhile. 41 enabled scheduled tasks still execute from linked worktrees, not this checkout (see OPERATIONS_DESIGN "What actually executes"). |
...
1. **Owner today:** publish `master` (Production source row) and restart the host before 17:00
   (Landing path row).
