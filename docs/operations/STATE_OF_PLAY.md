...
| Storage | Fresh September 5 09:49:34 disk metadata reports 38.16 GiB free, below the ordinary 50 GiB reserve; 34 GiB is the earlier 01:38 snapshot. No reclaim by this task or cause of the increase is established. The old 36 GiB hashing proposal is expired and binds spent v1, so capacity above 36 GiB cannot authorize production hashing for v2. The capture-host heavy window ended at 09:00. The mirror remains paused and unproved restorable; [item 325](../roadmap/items/item-325-tiered-data-retention-and-verified-archive-offload.md) owns the receipt and next admitted off-site path. |
| Off-site qualification | The 53 signed-in native tests passed. Actual v2 failed at September 5 10:09 Toronto with `rclone_create_only_copy_failed`; its self-hash and source binding validate. Real credential/config/root checks, initial source hash, compression and destination absence passed; copy/cryptcheck/restore remain unqualified. Preserve spent v1/v2. Repair `5e9b60e9d9f346346c0d8ef7de751fc43130d402`, [PR 22](https://github.com/michaelbooth1/weather/pull/22), shortens the invalid 19-byte suffix to fit rclone's 16-byte limit. It passed independent review, 58 workstation checks including native rclone, and changed-file compilation. A fresh v3 runner passed four smoke checks and requires all 59 signed-in fixtures before local staging. Its real result is pending; item 325 owns evidence. No upload, restore, deletion or reclaim is proved. |
...
1. Complete the prepared signed-in v3 qualification and review its fresh result
   under item 325. Preserve v1/v2 and qualify encrypted transfer and restore
   before exact-manifest reclaim. Any
   production hashing needs fresh admission; the expired proposal cannot substitute.
