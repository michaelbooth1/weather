...
| Off-site qualification | Signed-in v3 passed all 59 native checks and real local staging at September 5 10:52:24 Toronto on repair `5e9b60e9d9f346346c0d8ef7de751fc43130d402` ([PR 22](https://github.com/michaelbooth1/weather/pull/22)). All 13 staging checks passed; separate workstation verification confirmed both self-hashes and the 136,770,431-byte ciphertext hash. The manifest and receipt are copied to private Drive storage; ciphertext transfer and independent download/restore remain pending. The stage remains a provisional mirror copy with production identity unproved and no cleanup/deletion authority. Preserve v1-v3 evidence; item 325 owns the immutable pins. No reclaim is established. |
...
1. Transfer the verified v3 ciphertext under item 325, then independently download
   and qualify its restore. Local staging already passed; preserve v1-v3 evidence.
