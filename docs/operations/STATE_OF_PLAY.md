...
| Active paper attempt | The first real one-market attempt failed closed because the production economics snapshot is v0.2 and the cumulative branch requires v0.3. It emitted zero quote permissions and zero live permission. Collect a fresh external v0.3 snapshot after 18:00; do not accept a baseline automatically. |
