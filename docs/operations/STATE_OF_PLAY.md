...
**Last rewritten: 2026-08-20 19:39 America/Toronto.** Read this first, then
...
| Integration procedure | Item 329 redesigns the failed whole-night freeze as immutable per-attempt evidence, reviewed single-successor recovery, and hash-bound merge/downstream proof. The first audit's PASS-path and recovery defects were repaired; a second audit then found false running-suite alerts, a disabled-task window loss, no terminal `MERGED_UNVERIFIED` reconciliation, a PASS/deadline race, and four low-cost hardening gaps. The isolated branch now carries all eight follow-ups, including a separate non-authorizing reconciliation receipt. Sixty-seven focused contracts are present; all 13 PowerShell files parse and pass the expanded semantic sweep, the changed Python tests compile, and direct wait/verdict/timestamp/status probes pass. Focused pytest execution is pending because this patch was written inside the protected 18:00-00:30 window. The exact full suite, merge, and explicit scheduler adoption remain open; no task has been registered or started. |
