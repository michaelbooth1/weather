...
| Off-site qualification | Signed-in v3 passed all 59 native checks and real local staging at September 5 10:52:24 Toronto on repair `5e9b60e9d9f346346c0d8ef7de751fc43130d402` ([PR 22](https://github.com/michaelbooth1/weather/pull/22)). All 13 staging checks passed; the intact ciphertext, manifest and receipt are in private Drive storage. The owner completed the independent download; admitted workstation verification proved its hash and distinct file identity. The actual controller receipt and all three attended-runner checks now pass. The restore CLI in [PR 23](https://github.com/michaelbooth1/weather/pull/23) passed full CI. Item 325 owns the immutable pins and exact signed-in command. Actual restore, production identity and reclaim remain open. |
...
1. Execute the qualified v3 restore under item 325 from the workstation's
   ordinary signed-in PowerShell session. Preserve v1-v3 evidence.
