...
| Runner bootstrap | The currently adopted v1 runner predates the offline qualification marker. A minimal roll-free bootstrap must set that marker and local-only Git controls before candidate Python starts, restore inherited state, pass review/CI, and land before hardening qualification. Unmerged code cannot grant its own safety premise. |
