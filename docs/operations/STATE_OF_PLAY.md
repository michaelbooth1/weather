...
**Last rewritten: 2026-08-14 13:21 (the combined Stage 0/1 tip awaits its exact suite; the
continuous public-tape repair and paper-only market-harvest lane are stacked behind it).** Read this first, then `ESTABLISHED_FINDINGS.md`
...
| Paper-only market harvest | The separate `market_harvest` permission lane is implemented and pushed on the combined Stage 0/1 parent. It can issue bounded paper quote permission from current International books and event state without claiming model edge or promotion, but cannot select `live-pilot`. After Stage 0/1 and the execution-tape repair land, refresh it onto production, run an exact-tip full suite, and merge it only through the guarded quiet-window path. A real active-market paper tick must emit permission before either Stage 1 mutation is eligible. |
