...
**Last updated: 2026-09-09 America/Toronto (bulk archive qualification).** Read this first; read
...
offload followed by local reclaim. The exact selected July archive may retain
20 GiB of capture reserve; other plans keep their existing floor. Ordinary
overnight, capture, memory, verification and exact-file deletion gates remain.
...
| Off-site qualification | The production chunk bridge is published at d8bdb7111992cf0847ba1852ff2cf51a1a4e89d0, [draft PR 45](https://github.com/michaelbooth1/weather/pull/45). Workstation follow-up passed 18 checks, including native encryption/decryption and the corrected refusal tests; item 325 binds all receipts. Private Drive connectivity passed a synthetic 86-byte roundtrip. No production archive transfer, production restore or deletion is proved. The earlier one-file provisional mirror restore does not change that boundary. |
...
| Storage | The selected July 16-31 metadata totals 5,584 files / 111,275,511,800 historically allocated bytes; this is a candidate estimate, with zero new archive reclaim. Consumer closure remains open: historical replay, Parquet provenance and settled-folder discovery need explicit off-site availability handling. Full-size incompressible chunks also need a separately qualified transfer strategy. [Item 325](../roadmap/items/item-325-tiered-data-retention-and-verified-archive-offload.md#september-9-production-chunk-transfer-qualification) owns the current evidence and limits. |
...
1. Finish production archive transfer capacity and explicit off-site history
   availability, then perform admitted encrypted transfer and complete restore.
   Preserve exact source identity and review consumer closure before reclaim;
   the size of an old folder does not make its files deletion-eligible.
