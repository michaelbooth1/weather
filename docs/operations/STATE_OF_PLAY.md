...
| Verification | Paper and no-network preparation exposed two deterministic defects: the run orchestrator rejected its documented 600-second proof TTL, then candidate preflight assumed one economics row per event although temperature events have one row per binary condition. Set-based repair `4e87112a9d0cd528fafcbc108346b29f4356e3d9` has green [CI](https://github.com/michaelbooth1/weather/actions/runs/33341072122); local verification passed 65 focused tests plus compile, and the real 11-condition/22-token evidence cleared all three economics-identity blockers. Any later documentation-only tip still needs its own exact-head green check before use. |
