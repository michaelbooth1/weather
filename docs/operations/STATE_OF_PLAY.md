...
| Off-site qualification | Signed-in v3 passed all 59 native checks and real local staging at September 5 10:52:24 Toronto on repair `5e9b60e9d9f346346c0d8ef7de751fc43130d402` ([PR 22](https://github.com/michaelbooth1/weather/pull/22)). All 13 staging checks passed; separate workstation verification confirmed both self-hashes and the ciphertext hash. The intact ciphertext, manifest and receipt are now in private Drive storage; object size, exact parents and owner-only permissions are verified. Chrome blocked the automated download, so manual download is required. The independent restore CLI in [PR 23](https://github.com/michaelbooth1/weather/pull/23) passed full CI; its attended runner passed review and two parser/import smoke checks. Item 325 owns the pins and pending actual-input preflight. Real download/restore, production identity and reclaim remain unproved. |
...
1. Complete the independent Drive download of verified v3 under item 325,
   then bind its receipt and qualify the attended restore. Preserve v1-v3 evidence.
   Production identity and reclaim gates remain open. Any production hashing needs
   fresh admission; the expired proposal cannot substitute.
