...
| Verification | Paper and no-network preparation exposed two deterministic defects: the run orchestrator rejected its documented 600-second proof TTL, then candidate preflight assumed one economics row per event although temperature events have one row per binary condition. Set-based repair `4e87112a9d0cd528fafcbc108346b29f4356e3d9` has green [CI](https://github.com/michaelbooth1/weather/actions/runs/33341072122); independently re-reviewed fail-closed row-shape/cardinality hardening is `045a9e6d991db487f9feb9302e3450a864095aa9`. Local verification passed 73 focused tests plus compile, and the real 11-condition/22-token evidence conforms exactly. Any later documentation-only tip still needs its own exact-head green check before use. |
...
| Public substrate | Preserve every partial namespace. The first exposed a 267-character legacy Windows CAS failure. Short-root attempt `pilot-20260830T220702761Z` reached the now-repaired TTL defect. Attempt `pilot-20260830T225611906Z` passed public inputs, reviewed economics acceptance, and an 11-row paper run, then exposed the now-repaired multi-condition preflight defect. Its repair-verification receipt cleared those three blockers but correctly found that book/observation evidence had expired; do not reuse it. Later Stage refreshes use compact attempt-bound sibling roots below the legacy path budget. |
...
3. Build fresh attempt-local metadata, observation, weather/source, and CLOB
   evidence. Reuse the exact accepted economics snapshot only while its gate and
   hashes remain current; otherwise collect it again and stop for acceptance.
   Run the 600-second paper tick and no-network preflight, then stop for the exact
