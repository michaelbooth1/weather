...
| Unified International client | Official `polymarket-client==0.6.0` branch is prepared but behind production. Its real wheel contract passes and its rebate reader now requires the official pUSD proxy. Refresh it onto master and fold in the pushed v0.3 economics contract, install the exact live dependency only in the next admitted heavy window, then run one immutable full suite and quiet-window merge. |
