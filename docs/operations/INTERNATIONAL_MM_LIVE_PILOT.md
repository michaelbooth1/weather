...
- Select one central band whose current market spread, depth, fee eligibility,
  ...
  gates pass.
- Place exactly one smallest-valid backed post-only BUY for one TTL only.
...
before submission, then maintains the current bodyless heartbeat and exact
acknowledgment contract until the remaining TTL ends or authoritative lifecycle
evidence stops the order. The existing
