...
**Reconciliation note (2026-09-20, `codex/maker-reconcile-20260920`).** The
paragraphs above describe `master` before the maker candidate lands. The
reconciled maker branch carries the 2026-09-06 branch commits named above
(`c6ee36147`, `0baa30d7a`/`0cb942ee8`, `115e28398`, `ef75f358f`), so on any
tree that contains this note the Stage 0/1 code and the protocol sections
below describe the controls as exercised on 2026-09-06, not the original
ones. Adopting this branch is therefore the reviewed change that paragraph
waits for, and the three relaxations it lists need an explicit owner
disposition in that review. The Stage 0 structural scope and Stage 1
lifecycle design below must be present on the exact source selected for a
session; the portable source rule is owned by
[portable source authority](PORTABLE_LIVE_EXECUTION_HOST.md#source-authority).
Nothing in this note authorizes a live stage, and integration receipts do not
prove execution-host qualification.
...
described here. `capture_colocated_v1` is master-only: it requires exact
production-adopted canonical `master`, with
`HEAD == master == cached origin/master == live canonical refs/heads/master`.
For `portable_execution_v1`, use the current authority line in
[`STATE_OF_PLAY.md`](STATE_OF_PLAY.md) and fresh Git proof. On adopted master
the same equality applies. Under a named topic exception, require the exact
owner-authorized, reviewed, CI-green remote branch that authorization names:
local `HEAD`, its local branch tip, cached origin branch, and live canonical
branch tip must be identical, while local/cached/live canonical master are
synchronized and that master is an ancestor of the topic tip, under the full
