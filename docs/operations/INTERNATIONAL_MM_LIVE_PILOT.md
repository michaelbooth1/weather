...
.\venv\Scripts\python.exe -m weather.market.mm_live_pilot_cli doctor `
  ...
  --receipt-out (Join-Path $pilotPublicRoot ($pilotAttemptId + "-doctor-receipt.json")) `
...
the live boundary. The composer derives a candidate-bounded window of at most
120 seconds, requires at least 90 seconds still available immediately before
...
status-attestation helper closure. They rehash after acquiring those handles and
retain them through child exit. The parent sends cooperative cleanup at the
sealed stop, allows only the fixed 20-second cleanup grace, and then uses
kill-on-close containment if required.
