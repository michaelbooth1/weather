...
| W11 / item 325 | PARTIAL: v3 passed the 59-case signed-in suite, all 13 real staging checks and separate admitted evidence/hash verification. The intact ciphertext and safe JSON records are uploaded; exact size, parent folders and owner-only Drive permissions are verified. [Item 325](item-325-tiered-data-retention-and-verified-archive-offload.md#2026-09-05-1235-intact-private-upload-verified-independent-download-is-blocked) owns the immutable pins and restore source in [PR 23](https://github.com/michaelbooth1/weather/pull/23), whose full CI passed. The attended runner passed independent review and two parser/import smoke checks. Chrome blocked automated download; real download/restore, production identity and reclaim remain unproved. | Storage owner: complete the manual independent download, verify it on the workstation and bind the actual controller receipt before full CheckOnly and attended restore. Preserve v1-v3 and their source/output evidence. Production hashing requires fresh admission; the expired v1 proposal and paused mirror cannot substitute. Whole-W11 acceptance remains open, and the provisional stage grants no cleanup/deletion authority. |
