...
| W11 / item 325 | PARTIAL: signed-in v3 passed all 59 native checks in 4.00 seconds, then real local staging at 10:52:24 Toronto on repair `5e9b60e9d9f346346c0d8ef7de751fc43130d402` ([PR 22](https://github.com/michaelbooth1/weather/pull/22)). All 13 staging checks passed; a separate admitted workstation check verified manifest/receipt self-hashes, exact source binding and the fresh ciphertext hash. [Item 325](item-325-tiered-data-retention-and-verified-archive-offload.md#2026-09-05-independent-restore-source-published-intact-upload-awaits-manual-file-selection) owns immutable staging pins and the independent restore source/qualification in [PR 23](https://github.com/michaelbooth1/weather/pull/23). Safe JSON records are in private Drive storage; ciphertext upload needs manual file selection after connector/browser limits. Independent download/restore, production identity and reclaim remain unproved. | Storage owner: complete the intact upload, independently download it and bind the actual controller receipt before an attended canonical restore. Preserve v1-v3 and their source/output evidence. Production hashing requires fresh admission; the expired v1 proposal and paused mirror cannot substitute. Whole-W11 acceptance remains open, and the provisional stage grants no cleanup/deletion authority. |
