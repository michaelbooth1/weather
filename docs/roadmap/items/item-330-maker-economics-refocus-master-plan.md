...
| W11 / item 325 | PARTIAL: signed-in v3 passed all 59 native checks in 4.00 seconds, then real local staging at 10:52:24 Toronto on repair `5e9b60e9d9f346346c0d8ef7de751fc43130d402` ([PR 22](https://github.com/michaelbooth1/weather/pull/22)). All 13 staging checks passed; a separate admitted workstation check verified manifest/receipt self-hashes, exact source binding and the fresh ciphertext hash. [Item 325's v3 staging entry](item-325-tiered-data-retention-and-verified-archive-offload.md#2026-09-05-1052-signed-in-v3-staging-passes-cloud-ciphertext-transfer-remains-open) owns immutable pins and preserves v1/v2 history. Safe JSON records are copied to private Drive storage; ciphertext upload and independent download/restore remain pending. The provisional mirror copy does not prove production identity or grant cleanup/deletion authority; no reclaim is established. | Storage owner: transfer the verified ciphertext, independently download it and qualify the canonical restore path before considering production identity or manifest-bound reclaim. Preserve v1-v3 and their source/output evidence. Production hashing requires fresh admission; the expired v1 proposal and paused mirror cannot substitute. Whole-W11 acceptance remains open. |
