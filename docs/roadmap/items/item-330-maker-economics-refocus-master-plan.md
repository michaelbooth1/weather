...
| W11 / item 325 | PARTIAL: v3 passed the 59-case signed-in suite, all 13 real staging checks and separate admitted evidence/hash verification. The intact private Drive ciphertext was independently downloaded by the owner and transferred to the restricted workstation inbox; its hash and distinct file identity passed verification. The controller receipt and all three attended-runner checks now pass. [Item 325](item-325-tiered-data-retention-and-verified-archive-offload.md#2026-09-05-1320-independent-download-verified-attended-restore-qualified) owns the exact pins and signed-in command; restore source [PR 23](https://github.com/michaelbooth1/weather/pull/23) passed full CI. Actual restore, production identity and reclaim remain open. | Storage owner: execute the qualified attended restore from Michael's ordinary signed-in workstation PowerShell, then independently validate the resulting receipt. Preserve v1-v3 and their source/output evidence. Production hashing requires fresh admission; the expired v1 proposal and paused mirror cannot substitute. Whole-W11 acceptance remains open, and the provisional stage grants no cleanup/deletion authority. |
