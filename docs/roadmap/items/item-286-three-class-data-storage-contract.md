...
5. Document the operator rule: cleanup is allowed only from a reviewed cleanup
   ...
   only when their rebuild source and review proof are current.
...
- `ROADMAP.md` and this item file both mark the item `COMPLETE` with status text `COMPLETE 2026-06-23 - STORAGE CLASS REGISTRY AND REPORTING LIVE`.
