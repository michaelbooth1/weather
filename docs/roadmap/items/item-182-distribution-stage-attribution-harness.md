# 182. Distribution Stage-Attribution Harness [OPEN]
...
- [ ] Add `weather.reporting.distribution_stage_attribution` with schema
  registration and tests over the snapshot payloads.
- [ ] Emit per-stage marginal Brier/log-loss by hour and regime on the settled
  corpus.
- [ ] Run it in the daily refresh bundle and surface it in daily learning.
- [ ] Flag at least one net-negative stage with evidence.
