...
- Snapshot/source readiness is still blocked even though the loop itself recovered: `weather.collection.snapshot_tracker --status` reports `RUNNING`, `runtime_code_state = current`, `runtime_identity_matches_current = true`, capture liveness `OK`, snapshot cadence `PASS`, early-hour coverage `BLOCK`, and source-status proof `BLOCK` with `settlement_source_auth_failure`; optional provider status fields are false for both `optional provider source` and `optional provider source`.
...
- Source-status repair starts with free-source replacement verification, then `python -m weather.collection.snapshot_tracker --backfill-source-status --overwrite-source-status`, then keyless shadow/readiness reruns. Optional-provider source outages should stay outside the go-live path and should not be patched back into source code.
