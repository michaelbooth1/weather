...
Safe runtime and scenario work continued at `2026-06-27T14:13Z` to `14:17Z`:
...
- Superseding checks found the running CLOB loop had drifted back to undated `market_local_date` mode with `target_date = null`, price history/WebSocket capture enabled, and a process command line with no `--date`, no `--no-price-history`, and no `--no-websocket-events`. The latest `14:31Z` status has useful rows and fresh books, but per-market `target_date` remains null. The target-date audit after the undated restart passed only under startup-gap grace, so it is diagnostic rather than promotion-quality CLOB evidence.
- `snapshot_tracker --restart` started a fresh process, but follow-up status still returned `STALE_CODE`, `runtime_identity_matches_current = false`, dark/stale markets, snapshot cadence `BLOCK`, early-hour coverage `BLOCK`, and source-status proof `BLOCK` on `settlement_source_auth_failure` with Weather.com credential env vars absent/redacted.
...
- Before this probe, the documented source-status repair was run for the 12 current June 27 snapshot folders. It rewrote 3,756 local source-status rows from replay inputs and did not touch exchange state. This shell had `WEATHER_COM_API_KEY_present = false` and `WEATHER_COM_KEY_present = false`.
...
- Source-status proof summary remained `BLOCK`: `root_cause_class = settlement_source_auth_failure`, `source_status_blocked_market_count = 12`, `settlement_auth_failure_source_count = 12`, `weather_com_credential_present = false`, and `weather_com_credential_values_redacted = true`.
...
- Readiness `data/backtest/mm_live_readiness_20260627T122613_current_source_status_block.json`: `BLOCK` with 11 blockers, `live_capital_permission = false`, `current_counts_toward_live_forward_gate = false`, `snapshot_model_source_failing_gate_counts = {model_freshness: 6, source_status_degradation: 12}`, and Weather.com credential booleans false/redacted.
...
- Follow-up `snapshot_tracker --status` reported loop `RUNNING`, runtime identity current, `variant_missing_or_stale_market_count = 0`, `variant_markets_with_tape = 12`, source-status `BLOCK`, `root_cause_class = settlement_source_auth_failure`, `source_status_blocked_market_count = 12`, `settlement_auth_failure_source_count = 12`, `weather_com_credential_present = false`, `WEATHER_COM_API_KEY = false`, `WEATHER_COM_KEY = false`, redaction proof true, and early-hour coverage `BLOCK` for all 12 markets.
...
- Readiness `data/backtest/mm_live_readiness_20260627T124837_current_source_status_block.json`: `BLOCK` with 11 blockers, `live_capital_permission = false`, `current_counts_toward_live_forward_gate = false`, `snapshot_model_source_failing_gate_counts = {source_status_degradation: 12}`, `model_freshness_failed_market_count = 0`, `source_status_degradation_failed_market_count = 12`, and Weather.com credential booleans false/redacted.
...
- Readiness `data/backtest/mm_live_readiness_20260627T130826_current_source_status_block.json`: `BLOCK` with 11 blockers, `live_capital_permission = false`, `current_counts_toward_live_forward_gate = false`, `snapshot_model_source_failing_gate_counts = {source_status_degradation: 12}`, `model_freshness_failed_market_count = 0`, `source_status_degradation_failed_market_count = 12`, `source_status_blocker_root_cause_class = settlement_source_auth_failure`, Weather.com credential booleans false/redacted, and 0 quote/live permissions.
...
Collection/source-status robustness recheck after upstreaming provider-credential diagnostics:
...
Credential-aware source-status recovery recheck:
...
source_status_proof.summary.weather_com_credential_present = false
source_status_proof.summary.weather_com_credential_present_by_var = {WEATHER_COM_API_KEY: false, WEATHER_COM_KEY: false}
source_status_proof.summary.weather_com_credential_values_redacted = true
...
- A later snapshot-loop liveness regression briefly made the moving daily-roll folder fail freshness/preflight. Current snapshot tracker status is `RUNNING`, runtime code is current, heartbeat and latest snapshot age are fresh, but same-day snapshot cadence and early-hour coverage proofs remain `BLOCK` because of historical gaps.
...
- Snapshot/model/source status at the same time was still not quote-ready. `snapshot_tracker --status` reported the loop `RUNNING` and runtime identity current, but all 12 markets were `AT_RISK`; latest snapshots were about 17-19 minutes old, snapshot cadence proof was `BLOCK`, early-hour coverage proof was `BLOCK`, and source-status proof blocked all 12 markets because Weather.com/WU source families were degraded or unauthorized (`weather_forecast`, `wu_current`, `wu_history`; `wu_history` settlement auth failure on all 12). This is the current upstream explanation for model/source freshness fail-closed behavior.
...
- Status-output redaction was rechecked after broadening `redaction.py` to treat empty sensitive query values as unredacted until redacted. Capturing `snapshot_tracker --status` to an in-memory string and scanning it found 0 `apiKey=&` matches, 0 raw secret-query parameters, and 48 redacted secret-query parameters. A follow-up check in this shell found `WEATHER_COM_API_KEY_present = false` and `WEATHER_COM_KEY_present = false`, so the current Weather.com/WU auth blocker should be treated as external provider configuration/source-health evidence, not a reason to put credentials in the repo.
...
- Latest CLOB checks: after the `13:43Z` to `13:59Z` fixed-date recovery sequence, a later `14:13Z` status check found the loop had drifted back to `date_selection = market_local_date` with duplicate local-date loop processes. A second fixed-date restart at `14:15Z` restored loop `RUNNING`, `date_selection = fixed_target_date`, `target_date = 2026-06-27`, heartbeat/books/features fresh, discovery sanity `PASS`, 0 consecutive errors, no error markets, and 12 nonzero-book markets. Later checks found local-date drift again; the latest status has useful rows but still reports `target_date = null` and process command lines without `--date`. The old local-date audit was insufficient around cross-timezone midnight; explicit `weather.market.market_microstructure audit --strict --date 2026-06-27` is the correct check, but it must be paired with a fixed-date loop and useful target-date rows before evidence is countable.
...
- Current snapshot tracker check: `weather.collection.snapshot_tracker --status` reports loop `RUNNING`, runtime identity current, 12/12 markets `COLLECTING`, and fleet snapshot cadence `PASS`; it is still not promotion-ready because early-hour coverage is `BLOCK` and `source_status_proof.status = BLOCK` with `root_cause_class = settlement_source_auth_failure` blocks all 12 markets. The latest active-window paper-live-forward one-shot no longer fails preflight on model freshness, but source-status proof still blocks quote permission before policy output can be interpreted.
- Source-status proof schema was tightened to `source_status_proof_v0.2` so the status payload exposes `status`, `root_cause_class`, `reason`, and boolean-only Weather.com credential diagnostics at `fleet_collection.source_status_proof` and in the summary copy. Current verified values are `status = BLOCK`, `root_cause_class = settlement_source_auth_failure`, `reason = source status blocked: trading=12 live=12 promotion=12 unknown=0`, `weather_com_credential_present = false`, `weather_com_credential_values_redacted = true`, with 0 raw secret-query parameters in status output. Fleet observability Markdown now renders `Settlement-auth failure sources`, `Weather.com credential present`, and `Weather.com credential values redacted`; the regenerated fleet report is `CRITICAL`.
- Latest readiness was regenerated from the 135932 active-window paper-live-forward run/score after adding non-secret provider credential diagnostics and recovering CLOB/observation-trigger liveness. The JSON/Markdown report `source_status_weather_com_credential_present = false`, `source_status_weather_com_credential_present_by_var = {WEATHER_COM_API_KEY: false, WEATHER_COM_KEY: false}`, `source_status_weather_com_credential_values_redacted = true`, `paper_score_freshness_status = PASS`, `snapshot_model_source_failing_gate_counts = {model_freshness: 3, source_status_degradation: 12}`, and 0 quote/live permissions; upstream `snapshot_tracker --status` reports the same credential booleans under `source_status_proof.summary.provider_credential_environment` / `weather_com_credential_*`. The first two next actions still require external Weather.com credential configuration before rerunning source-status backfill and paper-live-forward/readiness.
- Fleet SLO recovery now carries the same redacted credential facts at the per-market source-status layer. When a settlement-auth failure has `weather_com_credential_present = false`, the recovery row uses root cause `settlement_source_auth_failure_missing_weather_com_credentials`, owner `external Weather.com provider credentials`, and a repair sequence of external env configuration first, then `python -m weather.collection.snapshot_tracker --backfill-source-status --overwrite-source-status`.
- Regenerated fleet observability at `2026-06-27T12:14:27Z` remains `CRITICAL`, but the machine-readable and Markdown recovery path is now specific and readable: `live_forward_slo.first_blocker.root_cause = settlement_source_auth_failure_missing_weather_com_credentials`, `owner = external Weather.com provider credentials`, and the first repair command starts with `configure WEATHER_COM_API_KEY or WEATHER_COM_KEY outside the repo`. `source_status_proof` remains `BLOCK` with 12 blocked markets, 12 settlement-auth failures, `weather_com_credential_present = false`, and redaction enabled. The Broad Recovery Gates Markdown row now summarizes the duplicated source-status detail as `(x12)` instead of repeating the same long text for every market.
...
- The Streamlit Market Making cockpit now reads the latest target-date `mm_live_readiness*.json` artifact and renders a `Live Readiness` section with status, live-capital permission, blocker count, source-status root cause, Weather.com credential presence, summary rows, and sorted next actions. The focused app/readiness test verifies the cockpit surfaces `settlement_source_auth_failure`, `Weather.com credential present = false`, and the `WEATHER_COM_API_KEY` / `WEATHER_COM_KEY` external-configuration action.
...
- `docs/research/MM_SIMULATION_SCENARIO_COVERAGE_2026-06-27.md` maps each requested simulation scenario to current repo artifacts, regression tests, coverage status, and remaining blockers. Current conclusion remains NO-GO: the latest active-window paper-live-forward run, latest-five aggregate, and post-CLOB-recovery shadow drill all have 0 quote permissions and block before quote/fill/reward evidence can be interpreted.
- Latest CLOB recovery evidence: stale generated event metadata initially blocked a public CLOB capture; `location_config_refresh` plus `event_metadata_validation --target-date 2026-06-27` produced `data/backtest/event_metadata_validation_20260627_goal_recheck.json` with 12/12 markets `PASS`. A fixed-date CLOB capture then wrote 22 books/tokens per market, and explicit fixed-date restart temporarily restored loop status to `RUNNING`/discovery `PASS`; strict audit at `2026-06-27T13:53:49Z` returned `ok = true` with startup gaps ignored after the new loop cutoff. Later drift back to undated `market_local_date` mode supersedes that recovery for promotion-quality evidence.
...
- Superseding CLOB drift check: after the fixed-date recovery, later status/process inspection found the running CLOB loop back in undated `market_local_date` mode with `target_date = null` and price history/WebSocket capture enabled. The latest `14:31Z` status has useful rows and fresh books, but per-market `target_date` remains null; the process command line still has no `--date`, no `--no-price-history`, and no `--no-websocket-events`. The subsequent explicit target-date audit returned `ok = true` only under startup-gap grace after the undated loop start. This is diagnostic only and not promotion-quality CLOB evidence. A focused safeguard now preserves fixed `target_date` through `market_microstructure ensure` restarts and preserves status-derived target date/enrichment flags through the operations-dashboard CLOB restart path; `tests/market/test_market_microstructure.py tests/operations/test_runtime_utilities.py` passed with `55 passed`.
