...
| CLOB status | Superseding checks found the running CLOB loop had drifted back to `date_selection = market_local_date`, `target_date = null`, price history/WebSocket capture enabled, and a live process command line with no `--date`, no `--no-price-history`, and no `--no-websocket-events`. The latest `14:31Z` status now has useful rows and fresh books, but per-market `target_date` remains null. | Current CLOB loop is not promotion-quality fixed-date evidence. The code now preserves fixed `target_date` through `ensure` and operations-dashboard restarts, but the already-running undated loop still needs an explicit operator restart before new evidence counts. |
| Explicit CLOB audit for 2026-06-27 | `ok = true` at `2026-06-27T14:22:07Z`, but only because startup gaps were ignored after the new undated local-date loop start cutoff | Treat this as diagnostic only. A strict audit must pass after a fixed-date loop with useful rows before source-status-clean active evidence is countable. |
| Snapshot tracker | Latest status returned `STALE_CODE`, `runtime_identity_matches_current = false`, 3 dark markets (`dallas`, `denver`, `houston`), snapshot cadence `BLOCK`, early-hour coverage `BLOCK`, and source-status `BLOCK` | Snapshot source/cadence evidence remains an operational blocker; a clean current-source loop plus next active-day early-hour coverage is still required |
...
| --- | ---: | --- | --- | --- |
| Clean active day with fresh snapshots, CLOB, observation trigger, and exchange economics | `BLOCK` | Latest active-window paper-live-forward run `20260627T135932865534Z`; readiness `mm_live_readiness_20260627T135932_after_clob_recovery_source_status_block.json`; shadow comparator `20260627T135111048558Z` | CLOB, event metadata, observation trigger, and exchange economics can pass together, but source-status proof, residual model freshness, and countability still block all markets | Configure external Weather.com credentials, backfill source status, rerun active-window paper-live-forward and readiness |
...
| Stale CLOB rows or missing CLOB tape | `PASS` for fail-closed logic, `WARN` for current runtime drift, `WARN` for day countability | The loop degraded to zero books/tokens, recovered after fixed-date capture/restart, then drifted again to an undated local-date process at `14:21Z`; tests cover failure and preservation: `test_data_layer_live_gate_requires_target_day_clob_artifacts`, `test_data_layer_live_gate_rejects_derived_clob_without_raw_artifacts`, `test_audit_book_tape_flags_internal_gap_and_stale_tail`, `test_ensure_clob_loop_preserves_fixed_target_date_when_restarting`, `test_ops_monitor_restart_clob_preserves_status_config` | Live/paper readiness requires target-day raw CLOB evidence and fails closed on stale/missing tape; supervisor/UI restarts now preserve fixed-date config | Explicitly restart CLOB with `--date 2026-06-27 --no-price-history --no-websocket-events`, confirm useful fixed-date rows, then rerun strict audit before source-status-clean active evidence |
...
- Current CLOB loop drifted back to undated `market_local_date` mode after the fixed-date recovery; no new CLOB evidence is countable until a fixed-date restart produces useful rows and strict audit passes without relying only on startup-grace diagnostics.
- Snapshot cadence, runtime identity, and early-hour coverage are currently blocked for promotion-quality evidence.
