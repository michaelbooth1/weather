...
- **The tracked tree is dirty most nights.** `WeatherLocationConfigRefresh` rewrites
  ...
  Because the automatic commit normalizes line endings, rollback then writes back the exact pre-commit bytes it recorded
  (and logs each rewrite) before the strict SHA-256 check (2026-09-24); a regeneration that landed between preparation and
  rollback is overwritten with those recorded bytes and regenerated by the fleet's next refresh.
